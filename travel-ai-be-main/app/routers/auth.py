...
async def register_user(user_data: UserCreate, db: Session = Depends(get_db)):
    ...
    try:
        ...
        MAX_PWD_BYTES = 72
        encoded_pwd = user_data.password.encode("utf-8")

        if len(encoded_pwd) > MAX_PWD_BYTES:
            logger.warning(f"Password too long ({len(encoded_pwd)} bytes). Truncating to {MAX_PWD_BYTES} bytes.")
            encoded_pwd = encoded_pwd[:MAX_PWD_BYTES]

        safe_password = encoded_pwd.decode("utf-8", errors="ignore")
